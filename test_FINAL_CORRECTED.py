#!/usr/bin/env python3
"""
FINAL CORRECTED Testing Script
...
print("=" * 100)
print("FINAL CORRECTED MODEL TEST")
print("=" * 100)
print()

# Configuration - CORRECTED PATH
BASE_MODEL = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
ADAPTER_PATH = "collegeadvisor_model_FINAL"  # CORRECTED from collegeadvisor_model_macos
DEVICE = "mps" if torch.backends.mps.is_available() else "cpu"

# Check adapter exists
if not Path(ADAPTER_PATH).exists():
    print(f"❌ ERROR: Adapter not found at {ADAPTER_PATH}")
    print(f"\nMake sure you've run: python finetune_FINAL_CORRECTED.py")
    sys.exit(1)
...
def generate_response(question, max_tokens=200):
    """
    Generate response using CORRECT TinyLlama Zephyr format.
    MUST match training format exactly!
    """
    # CORRECT format with </s> token
    prompt = f"<|user|>\n{question}</s>\n<|assistant|>\n"
...
for i, test in enumerate(TEST_QUESTIONS, 1):
    ...
    has_data = any(char.isdigit() for char in response) if response else False
    
    # FIXED: Check length before accessing word[0]
...
non_empty = sum(1 for r in results if r["word_count"] > 0)

print(f"Total Tests:              {len(results)}")
print(f"Non-Empty Responses:      {non_empty}/{len(results)}")
...
if non_empty == 0:
    assessment = "FAILED - All responses empty"
    recommendation = "✗ Training failed - check training logs"
elif avg_score >= 75:
    ...
    recommendation = "⚠ Needs improvement - consider retraining with more epochs"
else:
    assessment = "POOR - Insufficient Quality"
    recommendation = "✗ Requires retraining with better data or parameters"
...
# Save results
output_file = "model_test_results_FINAL.json"
